...
export class OverlayService extends EventEmitter {
	...
	private registerOverlayEvents() {
		...
		this.overlayApi.on('game-launched', (event, gameInfo) => {
			this.log(
				`Game launched: Name: ${gameInfo.name} GameId: ${gameInfo.id} Supported: ${gameInfo.supported}`
			);
		...
		this.overlayApi.on('game-injection-error', (gameInfo, error) => {
			this.log(
				`game-injection-error: Name: ${gameInfo.name} GameId: ${gameInfo.id} Error: ${error}`
			);
		});

		this.overlayApi.on('game-injected', gameInfo => {
			this.log(
				`new game injected! Name: ${gameInfo.name} GameId: ${gameInfo.id}`
			);
		});
